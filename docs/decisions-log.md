...
nunca se usa `frontmatter.Post(body, **fm)`. La firma real es `Post(content, handler=None, **metadata)`, así que una clave `handler` en el frontmatter se interpretaba como handler de serialización y `frontmatter.dumps()` escribía ese string como **contenido total del archivo** (body y frontmatter perdidos en silencio), y una clave `content` lanzaba `TypeError`. Los tres sitios que reserializan una nota entera (`create_note`, `append_to_note`, `set_property`) usan `_build_post`, que asigna `post.metadata`. (Eran cuatro: `update_wikilinks` se borró en 2026-09 por no tener caller y tener defectos conocidos — commit `213aa50`.) Los otros dos escritores de `.md` —`remove_broken_wikilinks` y `_reconcile_vault_sync`— no pasan por acá a propósito: editan líneas del texto crudo y lo mandan a `_atomic_write_sync` sin re-parsear el frontmatter, así que ninguna clave puede colisionar con la firma de `Post`. Del lado de lectura, `frontmatter.loads()` tiene el mismo choque de kwargs: `load_post()` lo envuelve y cae a `frontmatter.parse()` si lanza `TypeError`, para que una nota editada externamente con esas claves no rompa `read_note` ni los scans de `vault_cache`.
