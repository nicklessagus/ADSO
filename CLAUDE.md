...
- **Descubrimiento de proyectos y áreas (`_get_existing_items`):** busca notas con `type: project-index` / `type: area-index` en todo el vault, pero solo incluye las que tienen el campo canónico `project:` / `area:` en el frontmatter. Notas en la raíz del vault (ej: dashboards Dataview, exploradores) que tengan `type: area-index` por error sin el campo `area:` son ignoradas — sin este filtro harían fallback al nombre del directorio padre (`vault` en Docker) y aparecerían como entradas duplicadas en los reportes.
