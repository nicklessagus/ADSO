...
- **Latencia observada (RPi4, ago-2026, vault de 86 notas):** scans del vault 0,02-0,41 s (no son el cuello de botella a esta escala); `classify` **1,6 s en caliente pero 6-7 s en la primera llamada tras un período idle** (DNS + handshake TLS); `compute_embedding` 1,2 s. Una captura tras horas de inactividad cuesta ~8 s, y crear un proyecto nuevo paga ese camino frío dos veces (una llamada para el proyecto, otra para la nota).
